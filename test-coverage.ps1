...
.DESCRIPTION
    ...
    file under src/ (the repo convention is that all .sln files live there),
...
.PARAMETER Solution
    ...
    Leave this empty (the default) to let the script find src/*.sln itself.
...
.EXAMPLE
    ./test-coverage.ps1
    Autodiscovers src/*.sln, runs everything, and opens the report locally.
...
param(
    # Optional explicit solution list. When empty, the script discovers src/*.sln.
    [string[]]$Solution,
...
try {
    ...
    # explicit -Solution list, honor it; otherwise find every src/*.sln.
    if ($Solution) {
        $solutionFiles = $Solution | ForEach-Object { Get-Item $_ }
    }
    else {
        $solutionFiles = Get-ChildItem -Path (Join-Path $PSScriptRoot "src") -Filter "*.sln"
    }
